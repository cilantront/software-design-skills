...
## ⭐ Web 端原型「单页应用（SPA）」规则（全站设计）

Web 端原型（无论后台管理或 C 端站点）**必须以单页应用（SPA）方式展示全站设计**：所有核心页面（含登录/注册等入口页 + 各功能页）放在**同一个 HTML 文件**内，通过侧边栏菜单点击在**同一应用外壳内切换显示**，而非把多个页面纵向长滚罗列、也不得拆成多文件。

**参考范例**：`landing-page/demos/客服知识库缺口捕捉器/assets/prototypes/kb-gap-catcher-prototype.html`（标题为纯产品名「KB-Gap Catcher 全站点原型」，单文件内含「登录页 + 侧边栏 + 7 个 `.page-section` 功能页」，点击侧边栏 `nav-item` 经 `switchPage(pageId, el)` 切换；登录页经 `doLogin()` 隐藏后进入主应用壳）。

### 改造要点
1. **单一应用外壳**：文件内只保留一个常驻壳 = 侧边栏（`.sidebar`）+ 主区（`.main-content`）。所有功能页作为 `.page-section` 嵌入主区，同一时刻只显示一个（`.active`）。
2. **页面切分与切换**：每个功能页包成 `<section class="page-section" id="page-xxx">`；CSS 用 `.page-section { display:none; } .page-section.active { display:block; }`。侧边栏菜单项 `<li class="nav-item" data-page="xxx" onclick="switchPage('xxx', this)">`，点击调用 `switchPage(pageId, navEl)`：先 `document.querySelectorAll('.page-section').forEach(p => p.classList.remove('active'))` 隐藏全部，再给 `#page-<pageId>` 加 `active`，并同步高亮对应 `nav-item`（`.active`）。
3. **入口页也纳入壳内**：登录 / 注册等入口页作为全屏覆盖层（如 `.login-page { height:100vh; }`）或首个 `.page-section`，通过 JS（如 `doLogin()`）隐藏入口层、显示 `#mainLayout` 主壳，不另开独立页面文件。
4. **与「铺满屏幕」规则叠加**：body 仍 `margin:0; height:100vh; overflow:hidden`；外壳 `width:100%; height:100vh; display:flex` 铺满；主区 / 各页内容区 `.page-body { flex:1; min-height:0; overflow-y:auto; }` 在屏内滚动，**绝不纵向长滚罗列多个页面**。
5. **不得保留外围装饰标题**：同铺满规则，去掉「站点名 + 端类型说明」横幅，首屏直接进入应用界面（入口页或默认功能页）。

> 改造后效果：打开原型即看到完整应用（含登录入口），点击侧边栏在站内切换各功能页，与「客服知识库缺口捕捉器」的单文件 SPA 一致；`landing-page/index.html` 弹窗 iframe 嵌入时也能交互切换。
